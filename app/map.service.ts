...
export class MapService {
  ...
  //remove selection of feature layer with id 'FeatureSelection'
  removeFeatureSelection() {
    //remove existing graphic
    const selectionLayer = this.map.findLayerById('FeatureSelection')
    if (selectionLayer) {
  ...
  initCommonFeatureLayer(layer: string, opacity, id: number, title): FeatureLayer {
    return new FeatureLayer({
      url: layer,
      id: 'feature-' + id,
      outFields: ["*"],
...
pickCustomThemeLayers(response, layer, key, queryParams, groupLayer) {
  response.subscribe(json => {
    //(layer: string, opacity, index: number, id: number, title)
    let feature = this.initCommonFeatureLayer(layer.dynimacLayerUrls + '/2', layer.opacity, key, layer.name);
